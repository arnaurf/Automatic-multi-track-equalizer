...
for iT = 1:nTracks
    Magnitudes(iT,:) = mean(squeeze(frameMagnitudes(:, iT, :)),1);
...
for i_track = 1:nTracks
    ...
    y_filtered{i_track} = eq_filter(x_original.samples{i_track}, filter.center(i_track,:), zeros(1, 10)+Q, Mask(i_track,:).*(-1), fs);
end
...
y_mixdown = y_mixdown./max(sum(y_mixdown,1));
...
x_mixdown = x_mixdown./max(sum(x_mixdown,1));
